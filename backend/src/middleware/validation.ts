...
export function validateUploadRequest(req: Request, res: Response, next: NextFunction) {
  ...
  if (!contentType || !contentType.startsWith('multipart/form-data')) {
    return next(new BadRequestException('Invalid content type. Expected multipart/form-data.'));
  }
...
export function validateUploadedFiles(req: Request, res: Response, next: NextFunction) {
  ...
  if (files.length > 20) {
    return next(new BadRequestException('You can upload a maximum of 20 images at once.'));
  }

  for (const file of files) {
    if (!file.mimetype.startsWith('image/')) {
      return next(new BadRequestException(`Invalid file type: ${file.originalname}. Only image files are allowed.`));
    }

    const maxSize = 50 * 1024 * 1024; // 50MB
    if (file.size > maxSize) {
      return next(new BadRequestException(`File too large: ${file.originalname}. Max size is 50MB.`));
    }
  }
